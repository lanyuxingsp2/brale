...
- *~ai~*:
  ...
  - ~multi_agent~: *Multi-Agent Role Configuration* (Core Feature).
    Brale breaks down market analysis into multiple specialized fields, and you can assign the most suitable AI model to each field:
    - ~indicator_provider~: *Indicator Analyst*. Responsible for interpreting mathematical indicators like RSI, MACD. Recommended to use a model with strong logical reasoning (e.g., DeepSeek).
    - ~pattern_provider~: *Pattern Analyst*. Responsible for identifying candlestick patterns (e.g., Double Bottom, Head and Shoulders).
    - ~trend_provider~: *Trend Analyst*. Responsible for judging large-cycle direction and filtering noise.
    - ~mechanics_provider~: *Market Mechanics Analyst*. Responsible for analyzing derivatives data like Funding Rate, Open Interest.
    /Hint: The provider name here must correspond to the ~id~ defined in your ~models~ list./
...
- *~prompts~*: *Prompt Command Center*.
  ...
  - ~system_by_model~: Customize system prompts for different models.
...
1. *Data Collection & Caching*
   ...
   - Before each decision round, indicator_provider / pattern_provider / trend_provider generate inputs based on the latest cache (if configured enabled and data available), without re-fetching from the exchange.
