...
- *~ai~*：
  ...
  - ~multi_agent~：*多特工分工配置*（核心功能）。
    Brale 将市场分析拆解为多个专业领域，你可以为每个领域指定最擅长的 AI 模型：
    - ~indicator_provider~：*指标分析师*。负责解读 RSI, MACD 等数学指标。建议使用逻辑推理能力强的模型（如 DeepSeek）。
    - ~pattern_provider~：*形态分析师*。负责识别 K 线形态（如双底、头肩顶）。
    - ~trend_provider~：*趋势分析师*。负责判断大周期方向，过滤噪音。
    - ~mechanics_provider~：*市场力学分析师*。负责分析资金费率、持仓量等衍生品数据。
    /提示：这里的 provider 名称必须对应你在 ~models~ 列表中定义的 ~id~。/
  - ~freqtrade~：*执行端对接配置*。
...
1. *数据收集与缓存*
   ...
   - 每轮决策前，indicator_provider / pattern_provider / trend_provider 基于最新缓存生成输入（若配置启用且数据可用），不再向交易所重复拉取。
