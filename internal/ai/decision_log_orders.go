...
// RecordOrder 实现 execution.Recorder，写入 live_orders。
func (s *DecisionLogStore) RecordOrder(ctx context.Context, order *execution.Order) (int64, error) {
	s.mu.Lock()
	db := s.db
	s.mu.Unlock()
	if db == nil {
...
func (s *DecisionLogStore) ListOrders(ctx context.Context, symbol string, limit int) ([]execution.Order, error) {
	s.mu.Lock()
	db := s.db
	s.mu.Unlock()
	if db == nil {
...
func (s *DecisionLogStore) SaveLastDecision(ctx context.Context, rec LastDecisionRecord) error {
	s.mu.Lock()
	db := s.db
	s.mu.Unlock()
	if db == nil {
...
func (s *DecisionLogStore) LoadLastDecisions(ctx context.Context) ([]LastDecisionRecord, error) {
	s.mu.Lock()
	db := s.db
	s.mu.Unlock()
	if db == nil {
	...
	for rows.Next() {
		...
		out = append(out, LastDecisionRecord{
			Symbol:    sym,
			Horizon:   horizon,
